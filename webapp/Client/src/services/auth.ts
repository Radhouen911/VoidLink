    ...
    if (localEncryptedKey) {
      ...
      try {
        ...
        if (
          ...
          console.log("Account exists but no keys found. Attempting repair/setup...");

          // Check if public key exists (if it does, we lost the private key = FATAL)
          try {
            const userInfo: any = await api.getUserByUsername(username);
            if (userInfo.data?.publicKey) {
              throw new Error("Account exists with a PUBLIC key, but you have no backup of the PRIVATE key. This account is unrecoverable.");
            }
          } catch (e) {
